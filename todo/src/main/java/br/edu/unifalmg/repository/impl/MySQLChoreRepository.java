...
public class MySQLChoreRepository implements ChoreRepository {
    ...
    public boolean update(Chore chore){
        ...
        try {
            int result = statement.executeUpdate(" UPDATE db.chore\n" +
                    " SET description = " + chore.getDescription() + ", isCompleted = " + chore.getIsCompleted() + ", deadline = "+ chore.getDeadline() +"\n" +
                    " WHERE id = " + chore.getId());
            if (result > 0) {
                return Boolean.TRUE;
            }
            return Boolean.FALSE;
        } catch (SQLException exception) {
            System.out.println("Error when update a chore on database");
        } finally {
            closeConnections();
        }
        return false;
